...
for system_id in unique_system_ids:
    ...
    ##drop all columns except time and totenergydeli
    system_df = system_df[['time','totenergydeli']]
    predictions = predictions[['time','totenergydeli']]

    cs_dict["history"] = system_df.astype(object).to_dict(orient='records')
    cs_dict["predictions"] = predictions.astype(object).to_dict(orient='records')
